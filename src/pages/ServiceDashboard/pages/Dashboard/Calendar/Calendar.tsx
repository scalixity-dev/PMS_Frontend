...
const Calendar: React.FC = () => {
    ...
    const reminders = useMemo<Reminder[]>(() => {
        ...
        const serviceRequestReminders: Reminder[] = mockServiceRequests.map(req => ({
            ...
            assigneeName: req.client, // Using client as assignee for now, or could be separate
            recurring: false
        }));
    ...
    return (
        <div className={`flex flex-col h-[calc(100vh-140px)] bg-transparent overflow-hidden mx-auto transition-all duration-300 ${sidebarCollapsed ? 'max-w-full' : 'max-w-7xl'}`}>
            {/* Fixed Header Section */}
            <div className="flex-none pb-0 z-20 backdrop-blur-sm">
                <CalendarHeader
                ...
                <div className="flex">
                    ...
                    {/* Weekdays */}
                    <div className="flex-1 grid grid-cols-7 mb-2 bg-[#7CD755] text-white rounded-t-lg overflow-hidden shadow-md">
                        {weekDays.map((day) => (
                            <div key={day} className="py-2 md:py-3 text-center text-xs md:text-sm font-semibold tracking-wider border-r border-white last:border-r-0">
                                {day}
                            </div>
                        ))}
                    </div>
