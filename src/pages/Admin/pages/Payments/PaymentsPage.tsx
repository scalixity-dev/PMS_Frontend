...
const mockSubscriptions: Subscription[] = [
    { id: '1', userName: 'Sarah Jenkins', email: 'sarah.jenkins@email.com', userType: 'Property Manager', planName: 'Pro', amount: 49.99, startDate: '2023-01-01', endDate: '2024-01-01', status: 'Active' },
    { id: '2', userName: 'Mike Ross', email: 'mike.ross@email.com', userType: 'Property Manager', planName: 'Basic', amount: 19.99, startDate: '2023-05-15', endDate: '2024-05-15', status: 'Active' },
    { id: '3', userName: 'John Doe', email: 'john.doe@email.com', userType: 'Property Manager', planName: 'Enterprise', amount: 99.99, startDate: '2022-10-01', endDate: '2023-10-01', status: 'Expired' },
    { id: '4', userName: 'Alice Electric Co.', email: 'alice@electricco.com', userType: 'Service Pro', planName: 'Pro', amount: 29.99, startDate: '2023-12-01', endDate: '2024-12-01', status: 'Active' },
    { id: '5', userName: 'Bob Repairs', email: 'bob@repairs.com', userType: 'Service Pro', planName: 'Basic', amount: 9.99, startDate: '2023-02-01', endDate: '2024-02-01', status: 'Active' },
    { id: '6', userName: 'FixIt Now', email: 'contact@fixitnow.com', userType: 'Service Pro', planName: 'Basic', amount: 9.99, startDate: '2023-01-10', endDate: '2023-02-10', status: 'Expired' },
];
...
const StatCard = ({ title, value, icon, colorClass, subValue }: { title: string, value: string | number, icon: React.ReactNode, colorClass: string, subValue?: string }) => (
    <div className="bg-white p-4 md:p-6 rounded-xl border border-gray-100 shadow-sm flex items-start justify-between gap-2">
        ...
        <div className={`p-2 md:p-3 rounded-lg ${colorClass} text-white shadow-sm flex-shrink-0`}>
            {icon}
        </div>
    </div>
);
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-gray-900">Payments & Subscriptions</h1>
                    <p className="text-gray-500 text-xs sm:text-sm mt-1">Manage revenue, billing, and subscription plans.</p>
                </div>
            ...
            <div>
                <h2 className="text-base md:text-lg font-bold text-gray-900 mb-4">Financial Overview</h2>
            ...
            <div>
                <h2 className="text-base md:text-lg font-bold text-gray-900 mb-4">Subscription Health</h2>
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-[#7BD747] border-b border-[#7BD747]">
                                ...
                                <th className="px-6 py-4 text-left text-xs font-semibold text-white uppercase tracking-wider hidden lg:table-cell">Start Date</th>
                            ...
                            {filteredSubs.length > 0 ? (
                                filteredSubs.map((sub) => (
                                    <tr key={sub.id} className="hover:bg-gray-50/50 transition-colors group">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-gray-500 text-sm hidden lg:table-cell">
            ...
            <div className="md:hidden space-y-4">
                {filteredSubs.length > 0 ? (
                    filteredSubs.map((sub) => (
                        <div key={sub.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
                            <div className="flex items-start justify-between gap-3">
                                ...
                                <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border flex-shrink-0 ${sub.status === 'Active' ? 'bg-green-50 text-green-700 border-green-100' :
                                    'bg-red-50 text-red-700 border-red-100'
                                    }`}>
                                    {sub.status === 'Active' ? <CheckCircle size={10} className="mr-1" /> : <XCircle size={10} className="mr-1" />}
                                    {sub.status}
                            ...
                            <div className="mt-3 pt-3 border-t border-gray-100 flex items-center justify-between">
                                ...
                                <span className="text-xs text-gray-400">Ends {new Date(sub.endDate).toLocaleDateString()}</span>
                            </div>
                        </div>
                    ))
                ) : (
                    <div className="bg-white rounded-xl border border-gray-100 p-8 text-center">
                        ...
                        <p className="font-medium text-gray-900">No subscriptions found</p>
                    </div>
                )}
                {/* Mobile Pagination */}
                <div className="flex items-center justify-between py-2">
                    <p className="text-sm text-gray-500">{filteredSubs.length} results</p>
                    <div className="flex items-center gap-2">
