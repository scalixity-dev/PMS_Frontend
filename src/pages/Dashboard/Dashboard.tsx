...
export default function Dashboard() {
  ...
  const show = {
    applications: !isTeamMember || canView('rental-applications'),
    accounting:   !isTeamMember || canView('accounting'),
    leaseFunnel:  !isTeamMember || canView('rental-applications'),
    maintenance:  !isTeamMember || canView('maintenance'),
    properties:   !isTeamMember || canView('property-units'),
    deposits:     !isTeamMember || canView('accounting'),
    expiring:     !isTeamMember || canView('rental-applications'),
    overdue:      !isTeamMember || canView('accounting'),
    calendar:     !isTeamMember || canView('calendar'),
  };
  ...
  return (
    <div className={`space-y-5 ${sidebarCollapsed ? 'max-w-full' : 'max-w-7xl'} mx-auto transition-all duration-300`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {show.applications && <div className="bg-white rounded-lg p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ backgroundColor: '#E7EEE9' }}>
                <svg className="w-5 h-5" style={{ color: '#617C6C' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {(show.accounting || show.leaseFunnel) && (
          ...
          {show.leaseFunnel && (
            <div className="bg-white rounded-lg p-6 shadow-sm">
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={() => navigate('/dashboard/leasing/leases')} className="text-sm text-[var(--color-primary)] hover:opacity-90 font-semibold">View All</button>
              </div>
              <div className="flex flex-col sm:flex-row items-center gap-8">
                <div className="h-64 flex-shrink-0 relative" style={{ width: '250px' }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={leasesFunnelData} cx="50%" cy="50%" innerRadius={60} outerRadius={90} paddingAngle={2} dataKey="value">
                        {leasesFunnelData.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.color} />))}
                      </Pie>
                    </PieChart>
                  </ResponsiveContainer>
      ...
      {(show.maintenance || show.properties) && (
          ...
          {show.maintenance && (
            <div className="bg-white rounded-lg p-6 shadow-sm lg:col-span-3">
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={() => navigate('/dashboard/maintenance/requests')} className="text-sm text-[var(--color-primary)] hover:text-blue-700 font-semibold">See All</button>
              </div>
          ...
          {show.properties && (
            <div className="bg-white rounded-lg p-6 shadow-sm lg:col-span-2">
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={() => navigate('/dashboard/properties')} className="text-sm text-[var(--color-primary)] hover:opacity-90 font-semibold">View All</button>
              </div>
              <div className="space-y-4">
                {properties.length === 0 ? (
                  ...
                  <div key={property.id} className="flex items-center justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-4">
                        <img src={property.image} alt={property.address} className="w-16 h-16 rounded-lg object-cover" />
                        <div>
      ...
      {(show.properties || show.accounting || show.deposits || show.expiring || show.overdue) && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {show.properties && (
            <div className="bg-white rounded-2xl p-6 shadow-sm">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-lg font-semibold text-gray-900">Properties & Units</h2>
                <button onClick={() => navigate('/dashboard/properties')} className="text-sm text-[var(--color-primary)] font-semibold">View All</button>
              </div>
              <div className="h-64 relative flex items-center justify-center">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={propertiesUnitsData} cx="50%" cy="50%" innerRadius={80} outerRadius={120} paddingAngle={0} dataKey="value" startAngle={90} endAngle={450}>
                      {propertiesUnitsData.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.color} />))}
                    </Pie>
                    <Tooltip
                  ...
                  <div key={index} className="flex items-center gap-3">
                    <div className="relative flex-shrink-0">
                      <svg className="w-28 h-28 transform -rotate-90">
                        <circle cx={center} cy={center} r={radius} stroke="#f0f0f0" strokeWidth={strokeW} fill="none" />
                        <circle cx={center} cy={center} r={radius} stroke={metric.color.includes('orange') ? '#f97316' : '#3b82f6'} strokeWidth={strokeW} fill="none" strokeDasharray={`${dash} ${circumference}`} strokeLinecap="round" />
                      </svg>
                      <div className="absolute inset-0 flex items-center justify-center">
                    ...
                    <div className="flex-1">
                      ...
                      {metric.amount && <p className="text-lg font-bold text-gray-900">{metric.amount}</p>}
                      <p className={`text-xs font-medium ${metric.change.startsWith('+') ? 'text-green-600' : metric.change.startsWith('-') ? 'text-red-600' : 'text-gray-500'}`}>{metric.change}</p>
                    </div>
                  </div>
                );
              })}
          ...
          {show.deposits && (
            <div className="bg-white rounded-2xl p-6 shadow-sm">
              ...
              <div className="h-48 relative flex items-center justify-center">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={depositsData} cx="50%" cy="50%" innerRadius={60} outerRadius={90} paddingAngle={2} dataKey="value">
                      {depositsData.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.color} />))}
                    </Pie>
                  </PieChart>
                </ResponsiveContainer>
                <div className="absolute inset-0 flex items-center justify-center">
              ...
              {show.expiring && (
                <div className="bg-white rounded-2xl p-4 shadow-sm">
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <button onClick={() => navigate('/dashboard/leasing/leases', { state: { filter: 'expiring' } })} className="text-sm text-[var(--color-primary)] hover:opacity-90 font-semibold">View All</button>
                  </div>
                  <div className="space-y-3">
                    {leases.length === 0 ? (
                      <p className="text-sm text-gray-500">No expiring leases found.</p>
                    ) : leases.map((lease, idx) => (
                      <div key={lease.id} onClick={() => navigate(`/dashboard/leasing/leases/${lease.id}`)} className="flex items-center justify-between cursor-pointer hover:bg-gray-50 rounded-lg p-2 -m-2 transition-colors">
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            <div className="w-2 h-2 rounded-full bg-gray-900 flex-shrink-0" />
                            <div>
              ...
              {show.overdue && (
                <div className="bg-white rounded-2xl p-4 shadow-sm">
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <button onClick={() => navigate('/dashboard/accounting/transactions')} className="text-sm text-[var(--color-primary)] hover:opacity-90 font-semibold">View All</button>
                  </div>
                  <div className="text-center py-8">
                    <p className="text-base font-semibold text-gray-900">
                      {Number(dashboardStats?.financial.overdueInvoicesCount || 0) === 0 ? 'No overdue invoices' : `${dashboardStats?.financial.overdueInvoicesCount || 0} overdue invoices`}
                    </p>
