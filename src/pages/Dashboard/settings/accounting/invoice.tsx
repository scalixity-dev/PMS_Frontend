...
interface InvoiceSettingsValues {
  ...
  oneTimeLateFee: boolean;
  dailyLateFee: boolean;
  gracePeriod: string;
  graceTime: string;
}

const InvoiceSettings: React.FC = () => {
  const { data, isLoading } = useGetSettingsSection<InvoiceSettingsValues>("accounting_invoice");
  const updateSettings = useUpdateSettingsSection<InvoiceSettingsValues>("accounting_invoice");

  const [form, setForm] = useState<InvoiceSettingsValues>({
    recurringDays: "Days",
    oneTimeLateFee: true,
    dailyLateFee: false,
  ...
  useEffect(() => {
    if (data?.values) {
      setForm({
        ...
        oneTimeLateFee: Boolean(data.values.oneTimeLateFee),
        dailyLateFee: Boolean(data.values.dailyLateFee),
        gracePeriod: data.values.gracePeriod ?? "Grace Period",
        graceTime: data.values.graceTime ?? "Time",
      });
    }
  }, [data]);

  const save = (next: InvoiceSettingsValues = form) => {
  ...
  return (
    <AccountingSettingsLayout activeTab="invoice">
      <section className="border border-[#E8E8E8] rounded-2xl bg-[#FBFBFB] px-6 py-5">
        ...
        <button className="text-[#486370] text-sm font-medium mt-1 mb-4 hover:underline">Learn more</button>

        <div className="relative max-w-[200px] mb-6">
          <select
            value={form.recurringDays}
            onChange={(event) => setForm((prev) => ({ ...prev, recurringDays: event.target.value }))}
        ...
        <Button
          className="bg-[#486370] hover:bg-[#3a505b] text-white px-8 py-2 rounded-lg font-medium text-sm"
          onClick={() => save()}
      ...
      <section className="border border-[#E8E8E8] rounded-2xl bg-[#FBFBFB] px-6 py-5 mt-4">
        ...
        <p className="text-sm text-gray-500 mt-1 max-w-3xl">
          The system will automatically generate the following late fee once the tenant&apos;s grace period has expired. Both fees may be simultaneously enabled which will cause the
        ...
        <div className="space-y-4 mb-2">
          <div className="flex items-center gap-3">
            <Toggle
              checked={form.oneTimeLateFee}
              onChange={(value) => {
                const next = { ...form, oneTimeLateFee: value };
                setForm(next);
                save(next);
              }}
            />
          ...
          <div className="flex items-center gap-3">
            <Toggle
              checked={form.dailyLateFee}
              onChange={(value) => {
                const next = { ...form, dailyLateFee: value };
                setForm(next);
                save(next);
              }}
            />
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6 mt-2">
          ...
          <div className="relative max-w-[200px] w-full">
            <select
              ...
              onChange={(event) => setForm((prev) => ({ ...prev, graceTime: event.target.value }))}
              className="w-full appearance-none bg-white border border-[#E8E8E8] text-gray-500 text-sm rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#7BD747]"
            >
              <option>Time</option>
