...
const ImportServicePros: React.FC = () => {
    ...
    const handleNext = async () => {
        if (currentStep === 2 && file) {
            ...
            // After mapping, actually import the service providers
            await handleImport();
        } else if (currentStep < 4) {
    ...
    const handleValidate = async () => {
        ...
        try {
            ...
            const result = await serviceProviderService.validateExcel(file);
    ...
    const handleImport = async () => {
        ...
        try {
            // Now actually import the service providers (this will validate and queue)
            // Pass field mappings and importFirstRow option
            const result = await serviceProviderService.importFromExcel(file, fieldMappings, importFirstRow);
            setImportResult(result);

            // Update validation errors with any new errors from import
            setValidationErrors(result.errors);

            // Check if import was successful (jobId indicates Service Pros were queued)
            const hasQueuedItems = result.jobId !== null;
            const hasErrors = result.errors.length > 0;
            const totalRows = result.total || 0;

            if (hasQueuedItems) {
                // Some or all service pros were queued successfully
                ...
                setError('All Service Pros failed to import. Please check the validation errors below.');
                setCurrentStep(3); // Go back to validation step to show errors
            } else if (totalRows === 0) {
                // No valid rows found
                setError('No valid Service Pros found in the file. Please check your Excel file format.');
                setCurrentStep(3);
            } else {
                // No errors but also no jobId - this shouldn't happen, but handle it
                setError('Import completed but no Service Pros were processed. Please try again.');
                setCurrentStep(3);
            }
        } catch (err) {
    ...
    const getActionButtonText = () => {
        ...
        switch (currentStep) {
            ...
            case 4: return "Import Service Pros";
            default: return "Next";
        }
    };
...
export default ImportServicePros;
