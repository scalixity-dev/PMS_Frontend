    ...
    return (
        <div className="w-full max-w-5xl mx-auto px-4">
            ...
            <p className="text-center text-gray-600 mb-8 max-w-2xl mx-auto">
                {hasErrors
                    ? 'Some service pros have validation errors. Service pros without errors have been queued for import.'
                    : importResult
                        ? `Successfully queued ${importResult.successful} ${importResult.successful === 1 ? 'service pro' : 'service pros'} for import.`
                        : 'Your service pros have been processed.'
                }
                {' '}
                ...
                <div className="bg-[#F3F4F6] p-4 space-y-3 max-h-[500px] overflow-y-auto">
                    {errors.length === 0 ? (
                        <div className="bg-white rounded-lg p-6 text-center">
                            ...
                            <p className="text-gray-600 mt-2">All service pros are ready to import.</p>
                        </div>
                    ) : (
                        errors.map((error) => (
                ...
                {errors.length > 0 && (
                    <div className="bg-yellow-50 border-t border-yellow-200 px-6 py-4">
                        <p className="text-sm text-yellow-800">
                            <strong>Note:</strong> Service Pros with errors will be skipped during import.
                            Please fix the errors in your Excel file and re-upload, or proceed to import the valid ones.
                        </p>
                    </div>
                )}
            </div>
