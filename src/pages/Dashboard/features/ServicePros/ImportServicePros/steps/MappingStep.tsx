...
const MappingStep: React.FC<MappingStepProps> = ({
    ...
    useEffect(() => {
        const fetchSystemFields = async () => {
            try {
                setIsLoading(true);
                const response = await serviceProviderService.getImportFields();
                ...
                const mappedFields = response.fields.map((field, index) => {
                    ...
                    if (!mappedHeader) {
                        ...
                        if (keyLower.includes('email')) {
                            mappedHeader = fileHeaders.find(h => h.toLowerCase().includes('email'));
                        } else if (keyLower.includes('phone') || keyLower.includes('mobile')) {
                            mappedHeader = fileHeaders.find(h => h.toLowerCase().includes('phone') || h.toLowerCase().includes('mobile'));
                        } else if (keyLower.includes('name')) {
                            mappedHeader = fileHeaders.find(h => h.toLowerCase().includes('name') && !h.toLowerCase().includes('property') && !h.toLowerCase().includes('unit'));
                        } else if (keyLower.includes('company')) {
                            mappedHeader = fileHeaders.find(h => h.toLowerCase().includes('company'));
                        }
                    }
