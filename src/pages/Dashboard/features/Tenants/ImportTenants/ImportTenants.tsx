...
const ImportTenants: React.FC = () => {
    ...
    const handleNext = async () => {
        if (currentStep === 2 && file) {
            ...
            // After mapping, actually import the tenants
            await handleImport();
        } else if (currentStep < 4) {
    ...
    const handleValidate = async () => {
        ...
        try {
            ...
            const result = await tenantService.validateExcel(file);
    ...
    const handleImport = async () => {
        ...
        try {
            // Now actually import the tenants (this will validate and queue)
            // Pass field mappings and importFirstRow option
            const result = await tenantService.importFromExcel(file, fieldMappings, importFirstRow);
            setImportResult(result);

            // Update validation errors with any new errors from import
            setValidationErrors(result.errors);

            // Check if import was successful (jobId indicates tenants were queued)
            const hasQueuedTenants = result.jobId !== null;
            const hasErrors = result.errors.length > 0;
            const totalRows = result.total || 0;

            if (hasQueuedTenants) {
                // Some or all tenants were queued successfully
                ...
                setError('All tenants failed to import. Please check the validation errors below.');
                setCurrentStep(3); // Go back to validation step to show errors
            } else if (totalRows === 0) {
                // No valid rows found
                setError('No valid tenants found in the file. Please check your Excel file format.');
                setCurrentStep(3);
            } else {
                // No errors but also no jobId - this shouldn't happen, but handle it
                setError('Import completed but no tenants were processed. Please try again.');
                setCurrentStep(3);
            }
        } catch (err) {
            setError(toFriendlyErrorMessage(err, 'We could not import these tenants. Please try again.'));
