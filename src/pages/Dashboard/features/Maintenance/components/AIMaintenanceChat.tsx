...
interface AIMaintenanceChatProps {
  isOpen: boolean;
  onClose: () => void;
...
const AIMaintenanceChat: React.FC<AIMaintenanceChatProps> = ({ isOpen, onClose, onFormDataReceived }) => {
  const [messages, setMessages] = useState<Message[]>([
    {
      role: 'assistant',
      content: "Hi! I'm here to help you create a maintenance request. Describe your issue in plain English and I'll fill out the form for you.\n\nFor example: \"My refrigerator is not cooling properly, it's completely warm. I need it fixed urgently.\""
    }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/ai/maintenance-chat`, {
        method: 'POST',
      ...
      if (data.formData && data.isComplete) {
        ...
        setMessages(prev => [...prev, {
          role: 'assistant',
          content: "Perfect! I've collected all the details. Click 'Add to Form' below to fill out the form, then you can review and make any changes before saving. 🎉"
        }]);
      }
    } catch (error) {
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            ...
            />
            <div>
              <h2 className="text-white font-semibold text-lg leading-tight">AI Maintenance Assistant</h2>
              <p className="text-xs mt-0.5" style={{ color: 'rgba(255,255,255,0.7)' }}>Describe your issue naturally</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 space-y-5 min-h-0 custom-scrollbar" style={{ backgroundColor: '#f9fafb' }}>
          ...
          {isLoading && (
            <div className="flex flex-col gap-1 items-start">
              ...
              <div className="px-4 py-3 flex items-center gap-1.5 shadow-sm" style={{ backgroundColor: ASSISTANT_BG, borderRadius: '1.25rem', borderTopLeftRadius: '4px' }}>
          ...
          {pendingFormData && !isLoading && (
            <div className="flex justify-start">
              <div
                className="px-4 py-3 flex flex-col gap-3 max-w-[80%] shadow-sm"
                style={{ backgroundColor: '#e8f5f0', border: `2px solid ${PRIMARY}`, borderRadius: '1.25rem', borderTopLeftRadius: '4px' }}
              >
                <p className="text-sm text-gray-800 font-medium">Ready to add to form!</p>
                <button
                  onClick={handleConfirmAdd}
                  className="text-white px-6 py-2 rounded-full font-medium transition-all duration-200 flex items-center justify-center gap-2 hover:opacity-90"
                  style={{ backgroundColor: PRIMARY }}
                >
        ...
        <div className="px-5 pt-2 pb-1 flex-shrink-0 bg-white">
          <p className="text-[10px] text-gray-400 text-center">AI responses may not be 100% accurate. Please review before saving.</p>
        </div>
        ...
        <div className="px-5 pb-5 pt-2 flex-shrink-0 bg-white">
          ...
          >
            <textarea
              ...
              placeholder="Describe your maintenance issue... (Enter to send)"
              disabled={isLoading}
            ...
            <button
              ...
              onMouseEnter={e => { if (input.trim() && !isLoading) (e.currentTarget as HTMLButtonElement).style.transform = 'scale(1.08)'; }}
