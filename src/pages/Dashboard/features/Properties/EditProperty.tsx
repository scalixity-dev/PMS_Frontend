...
const EditProperty: React.FC = () => {
  ...
  useEffect(() => {
    if (property) {
      ...
      setFormData({
        ...
        latitude: property.address?.latitude !== null && property.address?.latitude !== undefined
          ? Number(property.address.latitude)
          : null,
        longitude: property.address?.longitude !== null && property.address?.longitude !== undefined
          ? Number(property.address.longitude)
          : null,
      });
  ...
  const updateFormData = (key: string, value: any) => {
    ...
    if (!isDirty && !isInitializingRef.current) setIsDirty(true);
  };
                    ...
                    onPlaceChanged={() => {
                      if (autocompleteRef !== null) {
                        ...
                        if (place && place.geometry && place.geometry.location) {
                          ...
                          // Prevent the country/state "reset dependent fields" effects
                          // from clobbering the values we just resolved above.
                          isInitializingRef.current = true;
                          setFormData(prev => ({
                            ...prev,
