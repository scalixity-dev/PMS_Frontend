...
interface PropertiesFilterProps {
    ...
    onFiltersChange: (filters: { status: string[]; occupancy: string[]; propertyType: string[]; balance: string[] }) => void;
}

const PropertiesFilter: React.FC<PropertiesFilterProps> = ({ onSearchChange, onFiltersChange }) => {
    ...
    const handleClearAll = () => {
        setSearchQuery('');
        setSelectedFilters({ status: [], occupancy: [], propertyType: [], balance: [] });
        onSearchChange('');
        onFiltersChange({ status: [], occupancy: [], propertyType: [], balance: [] });
    };
        ...
        <div ref={dropdownRef} className="bg-[#3A6D6C] p-4 rounded-full flex items-center gap-4 mb-8 justify-between relative">
            <div className="flex items-center gap-4 flex-1 overflow-visible flex-wrap">
                ...
                <div className="flex items-center gap-3">
                    {['status', 'occupancy', 'propertyType', 'balance'].map((filterType) => (
