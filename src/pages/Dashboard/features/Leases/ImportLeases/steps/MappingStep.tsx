...
const MappingStep: React.FC<MappingStepProps> = ({
    ...
    useEffect(() => {
        const fetchSystemFields = async () => {
            try {
                setIsLoading(true);
                const response = await leasingService.getImportFields();
                ...
                const mappedFields = (response.fields || []).map((field, index) => {
                    ...
                    if (!mappedHeader) {
                        ...
                        if (keyLower.includes('property') || keyLower.includes('address')) {
                            mappedHeader = fileHeaders.find(h => h.toLowerCase().includes('property') || h.toLowerCase().includes('address'));
                        } else if (keyLower.includes('tenant') || keyLower.includes('name')) {
                            mappedHeader = fileHeaders.find(h => h.toLowerCase().includes('tenant') || h.toLowerCase().includes('name'));
                        } else if (keyLower.includes('start') || keyLower.includes('date')) {
                            mappedHeader = fileHeaders.find(h => h.toLowerCase().includes('start') || h.toLowerCase().includes('date'));
                        } else if (keyLower.includes('rent') || keyLower.includes('amount')) {
                            mappedHeader = fileHeaders.find(h => h.toLowerCase().includes('rent') || h.toLowerCase().includes('amount'));
                        }
                    }
