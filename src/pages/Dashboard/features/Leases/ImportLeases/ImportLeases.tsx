...
const ImportLeases: React.FC = () => {
    ...
    const handleNext = async () => {
        if (currentStep === 2 && file) {
            ...
            // After mapping, actually import the leases
            await handleImport();
        } else if (currentStep < 4) {
    ...
    const handleValidate = async () => {
        ...
        try {
            ...
            const result = await leasingService.validateExcel(file);
            ...
            setFileHeaders(result.headers || []); // Ensure headers are set
    ...
    const handleImport = async () => {
        ...
        try {
            // Now actually import the leases (this will validate and queue)
            // Pass field mappings and importFirstRow option
            const result = await leasingService.importFromExcel(file, fieldMappings, importFirstRow);
            setImportResult(result);

            // Update validation errors with any new errors from import
            setValidationErrors(result.errors);

            // Check if import was successful (jobId indicates leases were queued)
            // The backend returns successful: 0 because leases are queued, not immediately created
            const hasQueuedItems = result.jobId !== null;
            const hasErrors = result.errors.length > 0;
            const totalRows = result.total || 0;

            if (hasQueuedItems) {
                // Some or all leases were queued successfully
                ...
                setError('All leases failed to import. Please check the validation errors below.');
                setCurrentStep(3); // Go back to validation step to show errors
            } else if (totalRows === 0) {
                // No valid rows found
                setError('No valid leases found in the file. Please check your Excel file format.');
                setCurrentStep(3);
            } else {
                // No errors but also no jobId - this shouldn't happen, but handle it
                setError('Import completed but no leases were processed. Please try again.');
                setCurrentStep(3);
            }
        } catch (err) {
            const errorMessage = err instanceof Error ? err.message : 'Failed to import leases';
            setError(errorMessage);
