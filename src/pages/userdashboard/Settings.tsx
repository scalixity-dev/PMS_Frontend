    ...
    const settingsOptions = [
        {
            ...
            icon: (
                <div className="w-12 h-12 flex items-center justify-center shrink-0">
                    <svg width="44" height="44" viewBox="0 0 56 56" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Account settings icon">
            ...
            link: "/userdashboard/settings/account"
        },
        {
    ...
    return (
        <div className="min-h-screen bg-white p-6 lg:p-10">
            ...
            <nav aria-label="Breadcrumb">
                <ol className="flex items-center gap-2 text-xl font-medium mb-12 ml-1">
                    <li>
                        <Link to="/userdashboard" className="text-[var(--dashboard-accent)] font-semibold hover:opacity-80 transition-opacity">Dashboard</Link>
                    </li>
                    <li aria-hidden="true" className="text-[#1A1A1A] font-semibold">/</li>
                    <li className="text-[#1A1A1A] font-semibold" aria-current="page">Settings</li>
                </ol>
            </nav>

            {/* Main Settings Container */}
            <div className="bg-[#F4F4F4] border border-[#E5E7EB] rounded-[20px] overflow-hidden shadow-[0px_3.68px_3.68px_0px_rgba(0,0,0,0.2)]">
                <div className="px-8 py-4 border-b border-[#E5E7EB]">
                    <h1 className="text-3xl font-medium text-[#1A1A1A] tracking-tight">Settings</h1>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {settingsOptions.map((option, index) => (
                            <Link
                                key={index}
                                to={option.link}
                                className="flex items-center gap-6 bg-[#F0F0F6] px-4 py-10 rounded-[10px] border border-transparent shadow-[0px_4px_4px_0px_rgba(0,0,0,0.25)]  transition-all duration-300 group"
                            >
                                <div className="flex-shrink-0">
                                    {option.icon}
                                </div>
                                <div className="flex-grow">
                                    <h3 className="text-xl font-semibold text-[#1A1A1A] mb-1">{option.title}</h3>
                                    <p className="text-[13.5px] text-[#6B7280] leading-[1.4] font-medium max-w-[240px]">
                                        {option.description}
                                    </p>
                                </div>
                                <div className="flex-shrink-0">
                                    <div className="w-[38px] h-[38px] rounded-full border-[1.8px] border-[#1A1A1A] flex items-center justify-center transition-all duration-300 group-hover:bg-[#1A1A1A] group-hover:text-white">
                                        <ChevronDown size={24} className="text-[#1A1A1A] group-hover:text-white" />
                                    </div>
                                </div>
                            </Link>
                        ))}
