...
const Step4FinalDetail: React.FC<Step4Props> = ({ selectedCategory, selectedProblem, finalDetail, onSelect, onNext, onSkip }) => {
    ...
    return (
        <>
            ...
            {/* Detail Grid - Responsive columns and spacing */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6 max-w-xl mx-auto px-4 mb-4">
                {details.map((item) => (
                    <div key={item}
                        onClick={() => onSelect(item)}
                        className={`group cursor-pointer flex items-center gap-3 md:gap-4 bg-white px-3 md:px-4 py-2 rounded-lg border-2 transition-all duration-200 box-border h-[60px] md:h-[72px] ${finalDetail === item ? "border-[#7ED957] shadow-sm" : "border-gray-100 hover:border-gray-300"}`}
                    >
                        {/* Radio Button - Responsive sizing */}
                        <div className={`w-5 h-5 md:w-6 md:h-6 rounded-full border flex items-center justify-center flex-shrink-0 transition-colors ${finalDetail === item ? "bg-[#7ED957] border-[#7ED957]" : "border-gray-300 group-hover:border-[#7ED957]"}`}>
                            {finalDetail === item && <div className="w-2 h-2 md:w-2.5 md:h-2.5 rounded-full bg-white" />}
                        </div>
                        {/* Detail Text - Responsive sizing */}
                        <span className={`font-normal text-base md:text-lg ${finalDetail === item ? "text-[#1A1A1A]" : "text-gray-900"}`}>{item}</span>
                    </div>
                ))}
            </div>
