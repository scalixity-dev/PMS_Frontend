...
const SettingCard = ({
    ...
    if (card.links && card.links.length > 0) {
        return (
            <div
                className="rounded-lg bg-[#F0F0F6] border border-transparent shadow-[0px_4px_4px_0px_rgba(0,0,0,0.25)] px-4 py-10 cursor-pointer focus:outline-none self-start"
                onClick={onToggle}
                role="button"
                tabIndex={0}
                onKeyDown={(e) => {
            ...
            >
                <div className="flex items-center gap-6">
                    ...
                    <div className="flex-1 space-y-1">
                        <p className="text-xl font-medium text-[#1A1A1A]">{card.title}</p>
                        <p className="text-[13.5px] text-[#6B7280] leading-[1.4] font-medium max-w-[240px]">
                            {card.description}
                        </p>
                    </div>
                    <div className="w-[38px] h-[38px] rounded-full border-[1.8px] border-[#1A1A1A] bg-white flex items-center justify-center shadow-inner shrink-0">
                        <ChevronDown
                            size={24}
                ...
                {isOpen && (
                    <div className="pt-3 pl-18 space-y-1 text-sm font-semibold text-[var(--dashboard-accent)]">
                        {card.links.map((link) => (
                            <p
                                key={link.path}
    ...
    return (
        <Link
            to={card.link || "#"}
            className="flex items-center gap-6 bg-[#F0F0F6] px-4 py-10 rounded-[10px] border border-transparent shadow-[0px_4px_4px_0px_rgba(0,0,0,0.25)] transition-all duration-300 group self-start"
        >
            <div className="shrink-0">
                {card.icon}
            </div>
            <div className="flex-1">
                <h3 className="text-xl font-medium text-[#1A1A1A] mb-1">{card.title}</h3>
                <p className="text-[13.5px] text-[#6B7280] leading-[1.4] font-medium max-w-[240px]">
                    {card.description}
                </p>
            </div>
        ...
        <div className="min-h-screen bg-white p-6 lg:p-10">
            ...
            <div className="bg-[#F4F4F4] border border-[#E5E7EB] rounded-[20px] overflow-hidden shadow-[0px_3.68px_3.68px_0px_rgba(0,0,0,0.2)]">
                <div className="px-6 py-3 border-b border-[#E5E7EB]">
                    <h1 className="text-2xl font-medium text-[#1A1A1A] ">Settings</h1>
                </div>
