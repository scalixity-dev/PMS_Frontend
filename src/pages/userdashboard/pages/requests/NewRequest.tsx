        ...
        <div className="bg-[#F5F5F5] rounded-xl shadow-[0px_3.9px_3.9px_0px_#00000040] border border-gray-100 p-8 md:p-8 relative overflow-hidden">
            ...
            <div className="flex justify-between items-center relative z-10">
              {steps.map((step, index) => {
                ...
                if (step.id === 1) {
                  ...
                  else if (currentStep === 2) progressWidth = "50%"; // Halfway for sub-step
          ...
          {currentStep === 2 && (
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-2xl mx-auto px-4 mb-4">
                {["Landscaping", "Fencing & Roof", "Pool", "Parking"].map((item) => (
                  <div key={item}
                    onClick={() => setSelectedSubCategory(item)}
                    className={`group cursor-pointer flex items-center gap-4 bg-white p-4 rounded-xl border-2 transition-all duration-200 box-border h-[72px] ${selectedSubCategory === item ? "border-[#7ED957] shadow-sm" : "border-gray-100 hover:border-gray-300"}`}
                  >
                    ...
                    <span className={`font-medium text-lg ${selectedSubCategory === item ? "text-[#1A1A1A]" : "text-gray-900"}`}>{item}</span>
                  </div>
                ))}
              </div>
          ...
          {currentStep === 3 && (
            <div className="text-center py-20">
              ...
              <div className="flex justify-center gap-4">
                <button onClick={() => setCurrentStep(2)} className="px-8 py-3 rounded-xl border border-gray-200 font-bold hover:bg-gray-50 transition-colors">Back</button>
                <PrimaryActionButton onClick={() => setCurrentStep(4)} text="Next" className="bg-[#7ED957] hover:bg-[#6BC847]" />
