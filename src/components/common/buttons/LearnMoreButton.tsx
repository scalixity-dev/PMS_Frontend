...
const LearnMoreButton: React.FC<LearnMoreButtonProps> = ({ 
  ...
  const baseClassName = variant === 'hero'
    ? `inline-flex items-center justify-center w-36 h-14 gap-2 rounded-lg px-5 py-4 border-2 border-[#4B5563] bg-transparent opacity-100 rotate-0 font-heading font-medium text-lg leading-none whitespace-nowrap tracking-[0] text-black transition-colors transition-transform duration-200 hover:scale-105 hover:bg-[#1F2937] hover:text-white focus:outline-none focus:ring-2 focus:ring-[#1F2937]/40 ${className}`
