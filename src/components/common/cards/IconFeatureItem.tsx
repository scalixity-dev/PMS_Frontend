...
const IconFeatureItem: React.FC<IconFeatureItemProps> = ({ icon, text }) => {
  ...
  const processedIcon = React.isValidElement(icon)
    ? React.cloneElement(icon as React.ReactElement<any>, ({ strokeWidth: 1.5, size: 24, color: '#13696B' } as any))
    : icon;
  ...
  return (
    <div className="flex flex-col items-center text-center gap-3">
      ...
      <div className="relative w-14 h-14">
        ...
        <div className="relative z-10 w-14 h-14 flex items-center justify-center rounded-full bg-[#CFFBBF]">
          <span className="text-[#13696B]">{processedIcon}</span>
        </div>
      </div>
